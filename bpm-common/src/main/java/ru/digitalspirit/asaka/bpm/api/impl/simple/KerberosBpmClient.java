...
/**
 * Simple implementation of {BpmClient} which supports {@link org.apache.http.impl.auth.KerberosScheme} authentification.
 */
...
public class KerberosBpmClient implements BpmClient {
    ...
    public ServiceClient getServiceClient() {
        if (serviceClient == null) {
            serviceClient = new ServiceClientImpl(new SafeUriBuilder(rootUri).addPath(SERVICE_ENDPOINT).build(), httpClient);
        }
    ...
    public OrganisationClient getOrganisationClient() {
        if (organisationClient == null) {
            organisationClient = new OrganisationClientImpl(new SafeUriBuilder(rootUri).build(), httpClient);
        }
    ...
    private class KerberosHttpClient extends CloseableHttpClient {
        ...
        private static final boolean skipPortAtKerberosDatabaseLookup = true;
        ...
        private KerberosHttpClient(String user, String password, String domain, String kdc) {
            try {
                ...
                logger.error("Can't create Kerberos bpmn client!", e);
                throw new BpmApiException(e);
            }
        }
         ...
         */
        private File createKrb5Configuration(String domain, String kdc) throws IOException {
        ...
        private File createLoginConfiguration() throws IOException {
            ...
            return new PrivilegedAction<CloseableHttpResponse>() {
                ...
                public CloseableHttpResponse run() {
                    try {
                        ...
                        logger.error("privilegedExecute(final HttpUriRequest request, final HttpContext context)", e);
                        return null;
                    }
                }
            };
